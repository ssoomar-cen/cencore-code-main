...
const TABLE_SF_ID_COL: Record<string, string> = {
  ...
  building: "salesforce_id",
  invoice_item: "salesforce_id",
  measure: "salesforce_id",
  credential: "salesforce_id",
  commission_split_schedule: "salesforce_id",
  energy_program_team_member: "salesforce_id",
};
...
const TABLE_TENANT_COL: Record<string, string | null> = {
  ...
  commission_split_schedule: "tenant_id",
  energy_program_team_member: "tenant_id",
};
...
const TABLE_PK_COL: Record<string, string> = {
  ...
  commission_split_schedule: "css_id",
  energy_program_team_member: "ep_team_member_id",
};
...
const OBJECT_REQUIRED_FK_COLS: Record<string, string[]> = {
  ...
  commission_split_schedules: ["commission_split_id"],
  energy_program_team_members: ["energy_program_id"],
};
...
const FIELD_MAPS: Record<string, { soql: string; table?: string; map: (r: any) => Record<string, any> }> = {
  ...
  energy_programs: {
    ...
    map: (r) => ({
      ...
      pgm_id: r.pgmId__c, push_to_d365: r.Push_to_D365__c, send_contacts: r.Send_Contacts__c,
      related_contract_sf_id: r.Related_Contract__c,
      last_synced_at: new Date().toISOString(),
    }),
  },
  connections: {
  ...
  commission_splits: {
    soql: "SELECT Id, Name, Percentage__c, Total_Commission_for_Contract_Term__c, Commission_Percent__c, Commission_Type__c, Status__c, Role__c, Notes__c, Type__c, Opportunity__c, Contract__c, Commission_Recipient__c, Energy_Program__c FROM Commission_Split__c",
    map: (r) => ({
      ...
      amount: r.Total_Commission_for_Contract_Term__c, commission_percent: r.Commission_Percent__c,
      commission_type: r.Commission_Type__c, status: r.Status__c,
      role: r.Role__c, notes: r.Notes__c, split_type: r.Type__c,
      last_synced_at: new Date().toISOString(),
    }),
  },
  measures: {
  ...
  credentials: {
    table: "credential",
    soql: "SELECT Id, Name, Contact__c, Organization__c, Credential_Type__c, Cert_ID__c, Included_in_Resume__c, Valid_To__c, Certified_Date__c, Status__c, Comments__c, Status_Reason__c, Credentials__c, Credentials_Description__c, Password__c FROM Credentials__c",
    map: (r) => ({
  ...
  commission_split_schedules: {
    table: "commission_split_schedule",
  ...
  energy_program_team_members: {
    table: "energy_program_team_member",
...
const RELATIONSHIP_DEFS: Record<string, Array<{ sfField: string; fkColumn: string; lookupTable: string }>> = {
  ...
  events:            [{ sfField: "AccountId",        fkColumn: "account_id",        lookupTable: "accounts" },
                      { sfField: "WhoId",            fkColumn: "contact_id",        lookupTable: "contacts" }],
  contracts:         [{ sfField: "Organization_Name__c", fkColumn: "account_id",    lookupTable: "accounts" },
  ...
  cases:             [{ sfField: "AccountId",        fkColumn: "account_id",        lookupTable: "accounts" },
                      { sfField: "ContactId",        fkColumn: "contact_id",        lookupTable: "contacts" }],
  invoices:          [{ sfField: "Contract__c",      fkColumn: "contract_id",       lookupTable: "contracts" },
                      { sfField: "Energy_Program__c",fkColumn: "energy_program_id", lookupTable: "energy_programs" }],
  commission_splits: [{ sfField: "Opportunity__c",   fkColumn: "opportunity_id",    lookupTable: "opportunities" },
                      ...
                      { sfField: "Energy_Program__c",fkColumn: "energy_program_id", lookupTable: "energy_programs" }],
  connections:       [{ sfField: "Organization__c",        fkColumn: "account_id",           lookupTable: "accounts" },
                      { sfField: "Contact__c",             fkColumn: "contact_id",           lookupTable: "contacts" }],
  energy_programs:   [{ sfField: "Organization__c",  fkColumn: "account_id",        lookupTable: "accounts" },
  ...
  invoice_items:     [{ sfField: "Invoice__c",        fkColumn: "invoice_id",        lookupTable: "invoices" },
                      { sfField: "Energy_Program__c", fkColumn: "energy_program_id", lookupTable: "energy_programs" }],
  measures:          [{ sfField: "Org__c",            fkColumn: "account_id",        lookupTable: "accounts" }],
  credentials:       [{ sfField: "Contact__c",        fkColumn: "contact_id",        lookupTable: "contacts" },
...
salesforceRouter.post("/sync", async (req: Request, res: Response) => {
  ...
  const validObjects = ["accounts","contacts","opportunities","leads","activities","events",
    ...
    "invoice_items","buildings","commission_splits","measures","credentials",
    "commission_split_schedules","energy_program_team_members"];
