...
import { Plus, ListFilter, Activity, Building2, CheckCircle2, Zap } from "lucide-react";
...
const Projects = () => {
  ...
  const { data: projectStats } = useQuery({
    ...
    enabled: !!tenantId,
    queryFn: async () => {
      const [totalRes, activeRes, plannedRes, completedRes] = await Promise.all([
        ...
        supabase.from("project").select("project_id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "Planned"),
        supabase.from("project").select("project_id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "Completed"),
      ]);

      return {
        totalPrograms: totalRes.count ?? 0,
        activePrograms: activeRes.count ?? 0,
        plannedPrograms: plannedRes.count ?? 0,
  ...
  const plannedPrograms = projectStats?.plannedPrograms ?? 0;
  const completedPrograms = projectStats?.completedPrograms ?? 0;
  const uniqueOrganizations = new Set(projects.map(p => p.account?.name).filter(Boolean)).size;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          icon={Building2}
          description="With active programs"
        />
        <StatCard
          title="Team Assignments"
          value={(plannedPrograms + activePrograms).toString()}
          icon={Users}
          description={`${completedPrograms} completed`}
        />
      </div>
