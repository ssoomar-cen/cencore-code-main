...
const columns: Column[] = [
  ...
  { key: "status", label: "Status", render: (v) => <Badge variant={v === "open" ? "default" : "secondary"}>{v}</Badge> },
];

export default function OpportunitiesPage() {
  const { data, isLoading, create, update, remove } = useOpportunities();
  ...
  const filters: FilterConfig[] = useMemo(() => [
    ...
    { key: "status", label: "Status", options: oppStatusOptions },
  ], [stageOptions, oppStatusOptions]);
  ...
  const formFields: FormField[] = useMemo(() => [
    { key: "name", label: "Opportunity Name", required: true, section: "Opportunity Information" },
    { key: "description", label: "Description", type: "textarea", section: "Opportunity Information" },
    { key: "account_id", label: "Org Name", type: "select", section: "Opportunity Information", options: (accounts || []).map((a) => ({ label: a.name, value: a.id })) },
    { key: "lead_source", label: "Lead Source", section: "Opportunity Information" },
    { key: "contact_id", label: "Contact", type: "select", section: "Opportunity Information", options: (contacts || []).map((c) => ({ label: `${c.first_name} ${c.last_name}`, value: c.id })) },
    { key: "next_step", label: "Next Step", section: "Opportunity Information" },
    { key: "stage", label: "Stage", type: "select", section: "Opportunity Information", options: stageOptions },
    { key: "status", label: "Status", type: "select", section: "Opportunity Information", options: oppStatusOptions },
    { key: "amount", label: "Amount", type: "number", section: "Financials" },
    { key: "close_date", label: "Estimated Close Date", type: "date", section: "Financials" },
    { key: "probability", label: "Probability (%)", type: "number", section: "Financials" },
  ...
  return (
    <CrmDataTable
      ...
      headerFields={[
        { key: "account_id", label: "Org Name" },
        { key: "close_date", label: "Estimated Close Date" },
